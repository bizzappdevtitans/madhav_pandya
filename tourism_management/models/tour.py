...
class Tour(models.Model):
    ...
    tour_name= fields.Selection([('dubai', 'Dubai'), ('philipines','Philipines'), ('maldives', 'Maldives')],string="Tour Name",Tracking=True)
    # tour_name_id1= fields.Many2one(comodel_name="package_info")
    ...
    tour_packages_id= fields.Many2one(related="tour_name_id.country",string="Click for Packages")

    # tour_packages_info_id= fields.Many2one(related="tour_name_id.packages")
    ...
    information_count = fields.Integer(string="Information Count",compute="compute_count")

    message_follower = fields.Char("Enter the message here")

    activity_ids= fields.Char()
    message_ids=fields.Char()


    @api.depends("tour_details_ids")
    def compute_count(self):
        """this counts tour details and will show on smart buttons"""
        for record in self:

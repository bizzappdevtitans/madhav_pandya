...
class TourItenerary(models.Model):
    ...
    tour_description_ids= fields.Many2many(comodel_name="tour_description",string="Tour Description")
    ...
    def create(self, vals):
        ...
        if vals.get("reference_noo", ("New")) == ("New"):
            vals["reference_noo"] = self.env["ir.sequence"].next_by_code("tour_itenerary") or _(
                "New"
            )
    ...
    def date_constrains(self):
        ...
        for rec in self:
            if rec.prefer_end_date < rec.prefer_start_date:
                raise ValidationError('Sorry, Prefer end date Must be greater Than prefer start date...')
